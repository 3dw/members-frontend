...
import { ALL_LABELS, type LabelConfig } from '@/data/labels_config';
...
export function useLabels() {
  ...
  const loadLabelsFromFirebase = () => {
    ...
    onValue(labelsRef, (snapshot) => {
      ...
      if (data) {
        labels.value = Object.values(data) as LabelConfig[];
      } else {
  ...
  const initializeDefaultLabels = async () => {
    ...
    const labelsObject = ALL_LABELS.reduce((acc, label) => {
      acc[label.id] = label;
      return acc;
    }, {} as Record<string, LabelConfig>);
    
    await set(labelsRef, labelsObject);
  };

  // 新增標籤
  const addLabel = async (labelData: Omit<LabelConfig, 'id'>) => {
    loading.value = true;
    error.value = null;
  ...
  // 更新標籤
  const updateLabel = async (labelId: string, updates: Partial<LabelConfig>) => {
    loading.value = true;
    error.value = null;
  ...
  // 根據顏色獲取標籤
  const getLabelsByColor = (color: string) => {
