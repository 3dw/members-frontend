...
export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        const addReply = (index, text) => {
            if (!dataLoaded.value || !props.uid || text.trim() === '')
                return;
        ...
        const saveEditMessage = (index, newTitle, newText) => {
            if (!dataLoaded.value || !props.uid)
                return;
            const messageToEdit = messages.value[index];
            if (messageToEdit.uid !== props.uid)
                return;
            if (newText.trim() !== '') {
        ...
        const quoteMessage = (messageIndex) => {
            ...
            nextTick(() => {
                if (messageEditor.value) {
                    messageEditor.value.setQuotedText(quotedText);
                }
            });
        };
        // 當前激活的下拉菜單
      ...
      `;
        ...
        const handleCollapseMessage = (messageIndex) => {
            if (messages.value[messageIndex]) {
                messages.value[messageIndex].textExpanded = false;
            }
        };
        const handleExpandReply = (messageIndex, replyIndex) => {
        ...
        const handleCollapseReply = (messageIndex, replyIndex) => {
            if (messages.value[messageIndex] && messages.value[messageIndex].replies) {
...
function __VLS_template() {
    ...
    __VLS_styleScopedClasses['bulletin-container'];
