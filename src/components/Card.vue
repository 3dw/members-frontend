  ...
  .hello
    router-link(v-if="clickable" :to="'/flag/'+ (h.uid || h.idx)")
      ...
      .content
        ...
        p.description(v-if="h.ask" v-html="'想學習：' + highlight(h.ask, mySearch)")
        p.description(v-if="h.price" v-html="'可支付：' + highlight(h.price, mySearch)")
      .content(v-if="h.connect_me && full")
        .ui.divider
        p.description(v-if="isEmail(h.connect_me)") 點擊寄信：
          a(@click.stop="gotoMail(h.connect_me)") {{h.connect_me}}
        p.description(v-else) 聯絡方式： {{h.connect_me}}
        p.description(v-if="h.freetime") 方便時間： {{h.freetime}}
      .content
        .ui.divider
        p(v-if="h.site")
          a(@click.stop="goto(h.site)") 個人網站：
            img(:src="'https://www.google.com/s2/favicons?domain=' + h.site" title='個人網址' alt='個人網址')
            i.right.arrow.icon
        p(v-if="h.site2")
          a(@click.stop="goto(h.site2)") 社群網站：
    ...
    div(v-else, :to="'/flag/'+ (h.uid || h.idx)")
      ...
      .content
        ...
        p.description(v-if="h.share" v-html="'可分享：' + highlight(h.share, mySearch)")
        p.description(v-if="h.ask" v-html="'想學習：' + highlight(h.ask, mySearch)")
        p.description(v-if="h.price" v-html="'收費：' + highlight(h.price, mySearch)")
      .content(v-if="h.connect_me && full")
        .ui.divider
        p.description(v-if="isEmail(h.connect_me)") 點擊寄信：
          a(@click.stop="gotoMail(h.connect_me)") {{h.connect_me}}
